...
\begin{tabularx}{\textwidth}{cXc}
	...
	\tableheader{3}{\textbf{ID} & \textbf{Beschreibung} & \textbf{Prio}}
    \accid{acc:s-metrics} & Ich kann statische Metriken der Code-Qualität, wie z.B. Code-Coverage, auf die Farbe der Blätter anwenden. & 1\\
    \accid{acc:s-hotspots} & Ich kann Ausreißer der ausgewählten Metrik gut erkennen. & 1\\
    \accid{acc:s-metric-value} & Ich kann mir die genaue Zahl der aktuellen Metrik für eine bestimmte Klasse anzeigen lassen & 1\\
    \accid{acc:s-overview} & Ich möchte möglichst viele Blätter gleichzeitig betrachten können. & 1\\
\end{tabularx}
...
\begin{tabularx}{\textwidth}{cXc}
	...
	\tableheader{3}{\textbf{ID} & \textbf{Beschreibung} & \textbf{Prio}}
    \accid{s-hotspots} & Die Struktur der Bäume entspricht der der Software. & 1\\
    \accid{acc:s-structure} & Die Struktur der Bäume ist eindeutig und hat keine Überschneidungen. & 1\\
    \accid{acc:s-leaf-connections} & Wenn ich ein Blatt auswähle, dann kann ich die ein- bzw. ausgehenden Abhängigkeiten hervorheben. & 2\\
    \accid{acc:s-thickness} & Die Dicke einer Kante oder Wurzel zeigt mir die Anzahl der dazugehörigen Abhängigkeiten. & 2\\
    \accid{acc:s-connection-highlight} & Wenn ich eine Kante oder Wurzel auswähle, dann kann ich die von den Abhängigkeiten betroffenen Blättern hervorheben. & 2\\
    \accid{acc:s-connection-count} & Wenn ich eine Kante oder Wurzel auswähle, dann kann ich sehen wie viele Abhängigkeiten in welche Richtung fließen. & 2\\
    \accid{acc:s-timestamp} & Ich kann den Zeitpunkt auswählen, auf die sich die dargestellten Daten beziehen. & 2\\
    \accid{acc:s-cyclic} & Zyklische Verbindungen können hervorgehoben werden. & 3
\end{tabularx}
...
\begin{tabularx}{\textwidth}{cXc}
	...
	\tableheader{3}{\textbf{ID} & \textbf{Beschreibung} & \textbf{Prio}}
    \accid{acc:s-drilldown} & Wenn ich eine Kante auswähle, dann kann ich das repräsentierte Softwareartefakt als neuen Waldboden festlegen. & 2\\
    \accid{acc:s-reset} & Ich habe die Möglichkeit den Wald zurückzusetzen. & 2\\
\end{tabularx}
...
\begin{tabularx}{\textwidth}{cXc}
	...
	\tableheader{3}{\textbf{ID} & \textbf{Beschreibung} & \textbf{Prio}}
    \accid{acc:d-metrics} & Ich kann Antwortzeiten und Laufzeitfehler auf die Blattfarbe anwenden. & 1\\
    \accid{acc:d-hotspots} & Ich kann Ausreißer der ausgewählten Metrik gut erkennen. & 1\\
    \accid{acc:d-metric-value} & Ich kann mir die genaue Zahl der aktuellen Metrik für eine bestimmte Klasse anzeigen lassen & 1\\
    \accid{acc:d-leaf-connections} & Wenn ich ein Blatt auswähle, dann kann ich die ein- bzw. ausgehenden Aufrufe hervorheben. & 2\\
    \accid{acc:d-thickness} & Die Dicke einer Kante oder Wurzel zeigt mir die Anzahl der dazugehörigen Aufrufe. & 2\\
    \accid{acc:d-connection-highlight} & Wenn ich eine Kante oder Wurzel auswähle, dann kann ich die von den Aufrufen betroffenen Blättern hervorheben. & 2\\
    \accid{acc:d-connection-count} & Wenn ich eine Wurzel oder Kante anklicke, dann kann ich sehen wie viele Verbindungen in welche Richtung gehen. & 2\\
    \accid{acc:d-timespan} & Ich kann den Zeitraum auswählen, auf die sich die dargestellten Daten beziehen. & 2
\end{tabularx}
...
\begin{tabularx}{\textwidth}{cXc}
	...
	\tableheader{3}{\textbf{ID} & \textbf{Beschreibung} & \textbf{Prio}}
    \accid{acc:d-overview} & Wenn ich die Laubfarbe des Waldes betrachte, dann möchte ich eine möglichst gute Gesamtübersicht über die ausgewählte Metrik haben. & 1\\
    \accid{acc:d-no-intersections} & Die Struktur der Bäume ist möglichst übersichtlich und hat so wenig Überschneidungen wie möglich. & 1
\end{tabularx}
...
\begin{tabularx}{\textwidth}{cXc}
	...
	\tableheader{3}{\textbf{ID} & \textbf{Beschreibung} & \textbf{Prio}}
    \accid{acc:u-project-selection} & Ich kann bereits gespeicherte Projekte öffnen. & 1\\
    \accid{acc:u-import} & Ich kann ein neues Projekt aus unterstützen Datenquellen importieren. & 2\\
    \accid{acc:u-add-data} & Ich kann Daten zu einem bereits gespeicherten Projekt aus unterstützen Daten hinzufügen. & 2\\
\end{tabularx}
...
Im weitern Verlauf des Textes wird bei der Umsetzung eines Akzeptanzkriteriums darauf in folgender Form verwiesen: \textit{(Akz. <ID>)}.
...
\begin{tikzpicture}
  ...
  \umluniassoc[geometry=|-|, mult=0..*, pos=2.6]{Record}{Association}
...
Aus User-Story 1 geht hervor, dass es bei dynamischen Daten gilt auch die Ressourcen-Auslastung zu visualisieren. Die Informationen dazu können im \textit{DynamicRecord} in dem Attribut \textit{Utilization} gespeichert werden. 
...
\begin{tikzpicture}
  ...
  \umlsimpleclass[y=-10, x=-1.8]{InnerNode}{
  }{}
  \umlsimpleclass[y=-10, x=1.8]{Leaf}{
  }{}
  ...
  \umlclass[y=-10, x=-5.7]{DirectConnection}{
    Key \\
    Start \\
    End \\
  }{}
  ...
  \umlunicompo[mult=1..*]{AggregatedConnection}{DirectConnection}
...
\begin{tikzpicture}
  ...
  \umlclass[y=-5, x=-8, type=abstract]{UiFloor}{
    Color \\
  }{}
  \umlclass{UiNode}{
    Key \\
  ...
  \umluniassoc[mult=0..*]{UiForest}{UiFloor}
...
%  \caption{Layout des Waldes von der Seite}
%  \label{fig:circle-packing-code-leaves}
%\end{figure}
%
%\begin{figure}[htb]
%  \includegraphics[width=\textwidth]{figures/circle-packing}
%  \caption{Layout des Waldes aus der Vogelperspektive mit visualisierten Circle-Packing}
%  \label{fig:layout}
%\end{figure}
...
Akzeptanzkriterium \ref{acc:s-thickness} und \ref{acc:d-thickness} wird demnach zwar im High-Fi Prototyp technisch erfüllt, muss aber mit realistischen Beispieldaten noch evaluiert werden. Es muss herausgefunden werden welche mathematische Funktion für die Dick der Kanten geeignet ist.
